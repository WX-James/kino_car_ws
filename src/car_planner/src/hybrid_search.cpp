...
void Car_KinoSearch::setParam(ros::NodeHandle& nh)
{
...
void Car_KinoSearch::setEnvironment(const EDTEnvironment::Ptr& env)
{
    ...
    for(double w = -car_w/2;w<=car_w/2;w+=resolution_/2){
        for(double l = 0;l<=car_l;l+=resolution_/2){
            for(double h = 0;h<=car_h;h+=resolution_/2){
                ...
                //     return true; 
                if(edt_environment_->sdf_map_->getInflateOccupancy(cur_p) == 1)
                {
                    return true;
                }
            }
