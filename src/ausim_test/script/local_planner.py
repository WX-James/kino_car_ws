...
class Local_Planner():
    def __init__(self):
        ...
        self.__pub_rtc_cmd = rospy.Publisher('/ackermann_steering_controller/cmd_vel', Twist, queue_size=10)                                      # cmd_vel的发布者，发布由MPC求解出的最优控制
        self._sub_odom = rospy.Subscriber('/state_ukf/odom', Odometry, self.__odom_cb)                              # 里程的订阅者，订阅里程计获得小车的位置
        self._sub_traj_waypts = rospy.Subscriber('/local_planner_node/mpc/traj_point', Float32MultiArray, self._vomp_path_callback)    # 期望轨迹的订阅者，订阅规划期发布的期望路点
    ...
    def _vomp_path_callback(self, data):
        if(len(data.data)!=0):
            ...
            # 拿到规划出来的轨迹点
            car_yaw = self.curr_state[2]    # 当前小车的yaw
            for i in range(size):
